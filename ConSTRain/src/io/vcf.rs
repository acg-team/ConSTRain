...
impl RepeatSource for VariantCallFile {
    ...
    fn load_repeats(&self, karyotype: &Karyotype, repeat_buffer: &mut Vec<TandemRepeat>, copy_number_buffer: &mut HashSet<usize>) -> Result<()> {
        ...
        let sample_idx = header
            ...
            .with_context(|| format!("Sample {} not found in file {}", self.sample_name, self.file_path))?;

        for record in bcf.records() {
            let record =
                record.with_context(|| format!("Error reading VCF record in file {}", self.file_path))?;
...
impl VariantCallFormatter {
    ...
    pub fn from_targets_lengths(sample_name: &str, targets: &[String], lengths: &[u64]) -> Result<Self> {
        if targets.len() == 0 || targets.len() != lengths.len() {
            bail!("Number of targets and lengths must be equal and greater than 0");
        }
        let header = make_vcf_header(targets, lengths, sample_name);

        Ok(Self { header })
    }
    pub fn repeats_to_sdtout(&self, tr_regions: &[TandemRepeat]) -> Result<()> {
